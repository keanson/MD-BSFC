...
import configs
import backbone
from data.datamgr import SimpleDataManager, SetDataManager, ConvDataManager
from methods.baselinetrain import BaselineTrain
from methods.baselinefinetune import BaselineFinetune
from methods.DKT import DKT
from methods.CDKT import CDKT
from methods.mirror import MD
from methods.mirrorGD import GD
from methods.protonet import ProtoNet
from methods.matchingnet import MatchingNet
from methods.relationnet import RelationNet
from methods.maml import MAML
from methods.sophia import SophiaG
...
def train(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params):
    ...
    if optimization == 'Adam':
        if params.method in ['CDKT', 'MD', 'GD']:
    ...
    elif optimization == "Sophia":
        if params.method in ['CDKT', 'MD', 'GD']:
            flag = model.get_negmean(params.mean)
            model.get_kernel_type(params.kernel)
            if flag:
                optimizer = SophiaG([{'params': model.model.parameters(), 'lr': 1e-4},
                                              {'params': model.feature_extractor.parameters(), 'lr': 1e-3},
                                              {'params': model.NEGMEAN, 'lr': 1e-4}])
            else:
                optimizer = SophiaG([{'params': model.model.parameters(), 'lr': 1e-4},
...
def converge(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params):
    print("Tot epochs: " + str(stop_epoch))
    if optimization == 'Adam':
        if params.method in ['CDKT', 'MD', 'GD']:
            flag = model.get_negmean(params.mean)
    ...
    # model.inner_loop(1, base_loader, optimizer)  # model are called by reference, no need to return
    ELBO_ls = []
    for i in range(30):
        loss = model.train_loop(i, base_loader, optimizer)
        ELBO_ls.append(-loss)
    torch.save(ELBO_ls, 'outer_ELBO_ls.pt')
...
if __name__ == '__main__':
    ...
    if params.method in ['baseline', 'baseline++']:
    ...
    elif params.method in ['MD', 'GD', 'CDKT', 'DKT', 'protonet', 'matchingnet', 'relationnet', 'relationnet_softmax', 'maml', 'maml_approx']:
        n_query = max(1, int(
        ...
        base_datamgr = ConvDataManager(image_size, n_query=n_query, **train_few_shot_params) #n_eposide=100
        base_loader = base_datamgr.get_data_loader(base_file, aug=params.train_aug)

        test_few_shot_params = dict(n_way=params.test_n_way, n_support=params.n_shot)
        val_datamgr = ConvDataManager(image_size, n_query=n_query, **test_few_shot_params)
        val_loader = val_datamgr.get_data_loader(val_file, aug=False)
        ...
        if(params.method == 'DKT'):
            ...
            model.init_summary()
        elif(params.method == 'MD'):
            model = MD(model_dict[params.model], **train_few_shot_params)
            model.init_summary()
        elif(params.method == 'GD'):
            model = GD(model_dict[params.model], **train_few_shot_params)
            model.init_summary()
    ...
    converge(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params)