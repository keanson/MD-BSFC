...
#python3 train.py --dataset="CUB" --method="MD" --train_n_way=5 --test_n_way=5 --n_shot=1 --train_aug --tau=1 --loss='ELBO' --steps=3 --seed=1
#python3 train.py --dataset="CUB" --method="MD" --train_n_way=5 --test_n_way=5 --n_shot=5 --train_aug --tau=1 --loss='ELBO' --steps=3 --seed=1

class MD(MetaTemplate):
    def __init__(self, model_func, n_way, n_support):
        super(MD, self).__init__(model_func, n_way, n_support)
        self.device = 'cuda:0'
        ## GP parameters
    ...
    def get_logits(self, x):
        ...
        with torch.no_grad():
            ...
            support_mu, support_sigma = self.predict_mirror(y_support, support_outputs, steps=30, rho=0.5)
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            if self.LOSS == 'ELBO':
                loss, logits = self.MirrorDescentELBO(y=y_train, output=output, steps=STEPS, REQUIRES_GRAD=True, temperature=self.TEMPERATURE)
                bs = len(y_train)
            else:
                ...
                loss, logits = self.MirrorPredictiveLoglikelihood(y_support, z_support, y_query, z_query, steps=STEPS, REQUIRES_GRAD=False, times=10000, tau=self.TEMPERATURE)
    ...
    def predict_mirror(self, y, output, steps=10, rho=0.5):
        times = 10000
        REQUIRES_GRAD = False
        ...
        with torch.no_grad():
            ...
            for step in range(steps):
                # get gradients (10) & (11)
                grad_mu, grad_v = self.GetGradientMeanParam(Y, vi_mean, vi_cov, times, temperature)
                # update lambda_tilde (9)
                theta_tilde_1 = (1 - rho) * theta_tilde_1 + rho * grad_mu
                theta_tilde_2 = (1 - rho) * theta_tilde_2 + rho * grad_v
                # transform params to traditional
                # N, C
                phi_tilde_1, phi_tilde_2 = self.ToTradition(theta_tilde_1, theta_tilde_2, 'natural')
                # update via (11) & (12)
                # C, N, N
                vi_cov = torch.linalg.inv(K_inv + torch.diag_embed(1/phi_tilde_2.T))
                vi_mean = (vi_cov @ torch.diag_embed(1/phi_tilde_2.T) @ phi_tilde_1.T.unsqueeze(2)).squeeze()
        return vi_mean, vi_cov 
    
    def montecarlo(self, q_posterior_list, times=10000, temperature=1, return_logits=False):
        samples_list = []
    ...
    def MirrorDescentELBO(self, y, output, steps=2, REQUIRES_GRAD=False, rho=0.5, times=10000, temperature=1):
        y = torch.tensor(y).long()
        ...
        #     theta_tilde_1 = self.theta_tilde_1.detach()
        #     theta_tilde_2 = self.theta_tilde_2.detach()
        # except:
        #     pass
        try:
            K_inv = torch.linalg.inv(output)
            ...
            for i in range(5):
                try:
                    K_inv = torch.linalg.inv(output + 0.1 * 10 ** i * torch.eye(N).reshape(1, N, N).to(self.device))
                    break
                except:
                    pass

        theta_tilde_1_ls = [theta_tilde_1]
        theta_tilde_2_ls = [theta_tilde_2]
        vi_mean_ls = [vi_mean]
        vi_cov_ls = [vi_cov]
        ...
        for step in range(steps):
            # get gradients (10) & (11)
            grad_mu, grad_v = self.GetGradientMeanParam(Y, vi_mean_ls[-1], vi_cov_ls[-1], times, temperature)
            # update lambda_tilde (9)
            theta_tilde_1 = (1 - rho) * theta_tilde_1_ls[-1] + rho * grad_mu
            theta_tilde_2 = (1 - rho) * theta_tilde_2_ls[-1] + rho * grad_v

            theta_tilde_1_ls.append(theta_tilde_1)
            theta_tilde_2_ls.append(theta_tilde_2)
            # transform params to traditional
            # N, C
            phi_tilde_1, phi_tilde_2 = self.ToTradition(theta_tilde_1_ls[-1], theta_tilde_2_ls[-1], 'natural')
            # update via (11) & (12)
            # C, N, N
            vi_cov = torch.linalg.inv(K_inv + torch.diag_embed(1/phi_tilde_2.T))
            vi_mean = (vi_cov @ torch.diag_embed(1/phi_tilde_2.T) @ phi_tilde_1.T.unsqueeze(2)).squeeze()
        ...
        # self.theta_tilde_1 = theta_tilde_1_ls[-1].detach()
        # self.theta_tilde_2 = theta_tilde_2_ls[-1].detach()
    ...
    def ToTradition(self, p1, p2, input_para='natural'): # convert natural or mean parameter to traditional parameter, single variate
        if input_para == 'natural':
            return -p1/2/p2, -1/2/p2
        elif input_para == 'mean':
            return p1, p2 - p1 ** 2

    def GetGradientMeanParam(self, y, vi_mean, vi_cov, times, temperature):
        # get gradients
        ## (10)
        N = y.shape[0]
        C = y.shape[1]
        ...
        samples_list = []
        for c in range(C):
            samples = MultivariateNormal(vi_mean[c], scale_tril=psd_safe_cholesky(vi_cov[c])).rsample(torch.Size((times, )))
            samples_list.append(samples)
        # times, N, C
        fn = torch.stack(samples_list).permute(1, 2, 0) / temperature
        # compute gradients
    ...
    def Inner(self, y, output, steps=50, REQUIRES_GRAD=False, rho=1, times=10000, temperature=1):
        y = torch.tensor(y).long()
        N = (self.n_support + self.n_query) * self.n_way
        C = self.n_way
        # N, C
        ...
        for step in range(steps + 1):
            ...
            ELBO2 = 0.5 * (torch.einsum('bii->b', K_inv @ vi_cov_ls[-1]) + (vi_mean_ls[-1].unsqueeze(1) @ K_inv @ vi_mean_ls[-1].unsqueeze(2)).squeeze() - N + torch.linalg.slogdet(output)[1] - torch.linalg.slogdet(vi_cov_ls[-1])[1] )
            ELBO -= torch.sum(ELBO2)
            ELBO_ngd_ls.append(ELBO.item())
        ELBO_ngd_ls.pop(0)
        
        # variational inference params
        vi_mean_ls = [vi_mean_ls[1]]
        vi_cov_ls = [vi_cov_ls[1]]
        ELBO_gd_ls = []
        ...
        for step in range(steps):
            ...
            ELBO2 = 0.5 * (torch.einsum('bii->b', K_inv @ vi_cov_ls[-1]) + (vi_mean_ls[-1].unsqueeze(1) @ K_inv @ vi_mean_ls[-1].unsqueeze(2)).squeeze() - N + torch.linalg.slogdet(output)[1] - torch.linalg.slogdet(vi_cov_ls[-1])[1] )
            ELBO -= torch.sum(ELBO2)
            ELBO_gd_ls.append(ELBO.item())

        return ELBO_ngd_ls, ELBO_gd_ls

    def inner_loop(self, epoch, train_loader, optimizer, print_freq=10):
        if self.STEPS == 'Annealing':
            STEPS = 1 + epoch // 50
        else:
            STEPS = self.STEPS
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            ELBO_ngd_ls, ELBO_gd_ls = self.Inner(y_train, output, steps=30, REQUIRES_GRAD=True, rho=0.005, times=10000, temperature=self.TEMPERATURE)
            break
            
        torch.save(ELBO_ngd_ls, 'ELBO_ngd_ls.pt')
        torch.save(ELBO_gd_ls, 'ELBO_gd_ls.pt')
        return 
