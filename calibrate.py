    ...
    def calibrate_cdkt(self, logits, labels):
        ...
        for i in range(iterations):
            for j in range(iterations):
                ...
                C = logits.shape[1]
                all_samples = torch.exp(logits / temperature_likelihood)
...
def get_logits_targets(params, validation=False):
    ...
    if params.method == 'baseline':
        ...
        model           = MD(model_dict[params.model], **few_shot_params)
    ...
    if params.save_iter != -1:
        ...
        split_str = split
    if params.method in ['maml', 'maml_approx', 'DKT', 'CDKT', 'MD']: #maml do not support testing with feature
...
def main():        
    ...
    torch.save(avg, params.dataset + '_' + str(temperature_2) + '_avg.pt')
